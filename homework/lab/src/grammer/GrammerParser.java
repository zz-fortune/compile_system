...
import semantics.SemanticsParser;
import semantics.Varible;
...
public class GrammerParser {
	...
	private List<String> warnings=new ArrayList<String>();	//	语法分析的警告信息列表
	...
	private Stack<String> signStack = new Stack<String>(); // 符号栈
	...
	public void parse(List<LexicalToken> tokens) {
		...
		while (!queue.isEmpty()) {
			...
			if (currentToken.getCategory().equals("ERROR")) { // 遇到非法字符直接跳过
				success=false;
				this.errors.add("Error at Line " + currentToken.getLineNum() + ": 非法字符"); // 记录错误信息
				queue.poll();
				continue;
			} else if (currentToken.getCategory().equals("NOTE")) { // 遇到符号表直接跳过
				continue;
			}
			if ((nextStep = actionTable[stateStack.peek()][terminals.indexOf(currentToken.toTerminal())]) == acc) { // 遇到 acc
																											// 则分析成功
				rules.add(null);
				queue.poll();
			} else if ((nextStep = actionTable[stateStack.peek()][terminals.indexOf(currentToken.toTerminal())]) > 0) { // 移进
				stateStack.push(nextStep - 1);
				signStack.push(currentToken.toTerminal());
				semanticsParser.pushNode(new Varible(currentToken.getWord(), currentToken.toTerminal(), currentToken.getValue()));
				queue.poll();
			} else if ((nextStep = actionTable[stateStack.peek()][terminals.indexOf(currentToken.toTerminal())]) < 0) { // 归约
				nextStep = -nextStep - 1;
				...
				semanticsParser.execAction(nextStep);
			} else { // 否则进入错误处理
				success=false;
...
//			}
//		}
		semanticsParser.backPatch();
		semanticsParser.printf();
	...
	private void errorhandle(Queue<LexicalToken> queue, Stack<Integer> stateStack, Stack<String> signStack) {
		...
		// 丢弃，直到遇到合法的 token
		while (!queue.isEmpty()) {
			if (!queue.peek().getCategory().equals("ERROR") && actionTable[stateStack.peek()][terminals.indexOf(queue.peek().toTerminal())] > 0) {
				break;
			}
			queue.poll();
		}
	...
	public void buildAnalysisTable(String grammer) {
		...
		semanticsParser = new SemanticsParser(100, this.grammers);
		semanticsParser.setGrammerParser(this);
		...
		computeFirsts();
		System.out.println("FISRT 集");
		System.out.println(this.firsts);
		computeLRfamily();
		System.out.println("LR（1）项目集");
		...
		for (int i = 0; i < this.LRfamily.size(); i++) {
					...
					for (int j = 0; j < this.grammers.size(); j++) {
						if (this.grammers.get(j).getLeft().equals(item.getLeft())
							...
							for (String string : item.getSuccessor()) {
								if (this.actionTable[i][this.terminals.indexOf(string)] >0 && (this.actionTable[i][this.terminals.indexOf(string)] <0 &&this.actionTable[i][this.terminals.indexOf(string)]!=-(j + 1))) {
			...
			for (String key : nexts.keySet()) {
				if (this.terminals.contains(key)) { // 如果是通过终结符可达，则填入 action 表
					if (actionTable[i][this.terminals.indexOf(key)] > 0
							&& actionTable[i][this.terminals.indexOf(key)] != this.LRfamily.indexOf(nexts.get(key))
									+ 1) {
	...
	/**
	 * 预处理文法。读入的文法是每行一个推导式，终结符包含的字母是小写，非终结符包含的字母是大写 空串用 {@code epsilon} 表示。
	 * 
	...
	public void addError(String info) {
		this.errors.add("Error at Line "+currentToken.getLineNum()+": "+info);
	}
	
	public void addWarning(String info) {
		this.warnings.add("Warning at Line "+currentToken.getLineNum()+": "+info);
	}
	...
	public List<CodeItem> getCodes() {
		return semanticsParser.getCodes();
	}
	...
	public Map<String, SignTableItem> getSignTable() {
		return semanticsParser.getSignTable();
	}
